...
const defaultState = () => ({
  ...
  triage: { symptom:'', severity:'', redFlags:[], status:'empty', submittedAt:null, actionStatus:'new', disposition:'', handledBy:'', handledAt:null, nurseNote:'', patientFeedback:'', review:{ status:'pending', outcome:'', rationale:'', redFlagAcknowledged:false, reviewedBy:'', reviewedAt:null, nurseTask:'', patientReceipt:'' } },
  scenario: 'routine',
...
function advance(next, name){ if(name==='PREVISIT_SUBMITTED' && !state.previsit.submittedAt) state.previsit.submittedAt=new Date().toLocaleString('vi-VN'); state.encounterState = next; event(name); save(); render(); }
...
function patient(){
  ...
  ${triagePanel()}${patientVoicePanel()}${decisionReceipt()}${active ? homePlan() : `<section class="empty card"><b>Chưa có kế hoạch tại nhà</b><p>Kế hoạch chỉ kích hoạt sau khi bác sĩ xác nhận và điều dưỡng hoàn tất teach-back.</p></section>`}`);
}
...
function resetScenario(kind){ const fresh=defaultState(); if(kind==='yellow'){fresh.triage={...fresh.triage,symptom:'Tiêu chảy',severity:3,status:'yellow',actionStatus:'new',submittedAt:new Date().toLocaleString('vi-VN')};} if(kind==='red'){fresh.triage={...fresh.triage,symptom:'Khó thở',severity:4,redFlags:['Khó thở khi nghỉ'],status:'red',actionStatus:'new',submittedAt:new Date().toLocaleString('vi-VN')}; fresh.alerts=[{type:'red',name:'TOXICITY_REPORTED_RED',detail:'Demo scenario red: khó thở khi nghỉ',at:new Date().toLocaleTimeString('vi-VN')}];} state={...fresh,role:state.role,scenario:kind}; save(); render(); }
...
function submitTriage(){ if(!state.triage.symptom)return; const red=state.triage.redFlags.length>0 || Number(state.triage.severity)>=4; state.triage.status=red?'red':Number(state.triage.severity)>=3?'yellow':'green'; state.triage.submittedAt=new Date().toLocaleString('vi-VN'); event('STRUCTURED_TRIAGE_SUBMITTED',{detail:`${state.triage.symptom} · mức ${state.triage.severity} · ${state.triage.status} · người bệnh cung cấp`}); save(); render(); }
...
function resolveEscalation(){ const r=state.triage.review; if(!r.outcome||!r.rationale.trim()||!r.redFlagAcknowledged)return; r.status='resolved';r.reviewedBy='BS. Mỹ Linh';r.reviewedAt=new Date().toLocaleString('vi-VN');r.nurseTask=r.outcome.includes('Liên hệ')?'Điều dưỡng liên hệ người bệnh và ghi kết quả.':'Điều dưỡng theo dõi theo hướng dẫn của cơ sở.';r.patientReceipt='Đội điều trị đã xem báo cáo và cập nhật bước tiếp theo.';state.triage.actionStatus='resolved';event('DOCTOR_ESCALATION_RESOLVED',{detail:`${r.outcome} · ${r.rationale}`});save();render(); }
