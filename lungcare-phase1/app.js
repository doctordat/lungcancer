...
function normalize(raw) {
  ...
  return {
    ...
    medicationSafety: { ...d.medicationSafety, ...(raw.medicationSafety || {}), reviewed:{...d.medicationSafety.reviewed,...(raw.medicationSafety?.reviewed||{})}, checks:Array.isArray(raw.medicationSafety?.checks)?raw.medicationSafety.checks:d.medicationSafety.checks },
...
function medicationSafetyBrief(){ const m=state.medicationSafety; return `<section class="card med-safety"><small>MEDICATION SAFETY BRIEF · DEMO CDS</small><h3>Osimertinib · review trước quyết định</h3><p>Không tự tính liều/không thay y lệnh. Bấm từng yếu tố để ghi nhận đã review.</p><div class="med-grid">${m.checks.map(c=>`<button class="med-check ${m.reviewed[c.id]?'reviewed':''}" onclick="reviewMed('${c.id}')"><span>${m.reviewed[c.id]?'✓':'○'}</span><div><b>${c.label}</b><strong>${c.value}</strong><small>${c.source}</small></div></button>`).join('')}</div><div class="warning">Tương tác thuốc chưa kết nối drug database · cần kiểm tra theo nguồn/protocol được cơ sở phê duyệt.</div></section>`; }
function reviewMed(id){ state.medicationSafety.reviewed[id]=true; event('MEDICATION_SAFETY_REVIEWED',{detail:`Đã review medication safety: ${id}`}); save(); render(); }
