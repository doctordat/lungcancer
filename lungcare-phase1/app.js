...
const defaultState = () => ({
  ...
  scenario: 'routine',
  careLoop: { plan:{planId:'PLAN-LC-871748-001',revision:1,status:'active',createdBy:'BS. Mỹ Linh',confirmedAt:null}, tasks:[{id:'med',label:'Uống thuốc hôm nay',status:'open'},{id:'symptom',label:'Kiểm tra triệu chứng',status:'open'},{id:'labs',label:'Nhắc xét nghiệm an toàn',status:'open'},{id:'followup',label:'Chuẩn bị tái khám',status:'open'}], symptomCheck:{completed:false,at:null,summary:''} },
...
function normalize(raw) {
  ...
  return {
    ...
    careLoop: { ...d.careLoop, ...(raw.careLoop || {}), plan:{...d.careLoop.plan,...(raw.careLoop?.plan||{})}, tasks:Array.isArray(raw.careLoop?.tasks)?raw.careLoop.tasks:d.careLoop.tasks, symptomCheck:{...d.careLoop.symptomCheck,...(raw.careLoop?.symptomCheck||{})} },
...
function carePlanCard(){ const p=state.careLoop.plan; return `<section class="card care-plan"><small>MY CARE · KẾ HOẠCH KẾT NỐI</small><h3>Plan ${p.planId} · revision ${p.revision}</h3><div class="plan-status">${pill(p.status==='active'?'Đang theo dõi tại nhà':'Chưa active','green')}${pill('Nguồn: quyết định BS · không phải y lệnh')}</div><p>Care loop: thuốc hôm nay · symptom check · xét nghiệm · tái khám.</p>${state.careLoop.symptomCheck.completed?pill(`Đã check · ${state.careLoop.symptomCheck.at}`,'green'):button('Hoàn tất symptom check hôm nay','completeSymptomCheck()','outline')}</section>`; }
